...
| Перезарядка после окончания/разрушения | 6 сек |
...
| Автоклик | 1 | `1` изотоп | требует Крепкое ядро; автоматически добавляет клик генерации каждые `0.35` сек при наведении на ядро |
| Быстрая сборка | 4 | `8 / 30 / 120 / 420` DNA | требует Крепкое ядро; `+25%` к скорости генерации снарядов кликами |
...
| Радикальный след | 3 | `60 / 220 / 700` DNA | требует ДНК-след; `+5%` шанса `+1` радикала за не-боссового врага за уровень |
...
| Swarm Reach | 2 | `5 / 16` радикалов | требует SwarmMolecule; `+3` дальности SwarmMolecule за уровень |
...
| Быстрая рекристаллизация | 3 | `250 / 750 / 1800` DNA | требует MembraneMolecule; `-20%` перезарядки мембраны за уровень |
...
Swarm-залпа.
Таланты HP ядра, автоклика ядра, скорости генерации снарядов кликами,
...
подбора ДНК, unlock SwarmMolecule, урона SwarmMolecule,
количества/ширины лучей SwarmMolecule, дальности SwarmMolecule,
